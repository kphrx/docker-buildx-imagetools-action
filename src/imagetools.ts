...
class StdErrError extends Error {
  constructor(stderr: string) {
    super(`buildx failed with: ${stderr.match(/(.*)\s*$/)?.[0]?.trim()}` ?? 'unknown error')
  }
}
...
async function createRaw(
  ...
  if (await toolkit.buildx.versionSatisfies('>=0.12.0') && opts.annotations != null) {
  ...
  const { exitCode, stdout, stderr } = await Exec.getExecOutput(cmd.command, cmd.args, {
...
export async function inspect(
  ...
  const cmd = await toolkit.buildx.getCommand(args)
  const { exitCode, stdout, stderr } = await Exec.getExecOutput(cmd.command, cmd.args, { ignoreReturnCode: true })
  if (stderr.length > 0 && exitCode !== 0) {
    throw new StdErrError(stderr)
  }
